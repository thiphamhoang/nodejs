const paymentMethodRequireModel = require('../paymentMethodModel');

const  {multipleMongooseToObject, mongooseToObject} = require('../../../../util/mongoose');
const deleteUtil =  require('../../../../util/deleteRecursive');
const errorUtil =  require('../../../../util/errorNoti');
const folder_module = 'paymentMethod';
const module_name = 'Phương thức thanh toán';
...
const permiView = 'paymentMethod_crm_view';
const permiEdit = 'paymentMethod_crm_edit';
...
class Controller {
    ...
    show(req, res,next){
        ...
        const paymentMethodModel = paymentMethodRequireModel.createCon(DBname,collection)
        Promise.all( [
            // catQuery,
            //chuc nang sortable(req) la goi tron model 
            paymentMethodModel.find({}), 
        ])
            .then(([paymentMethodList]) => res.render(folder_view + '/show',{
                title: module_name,
                itemList: multipleMongooseToObject(paymentMethodList),
                layout: layout,
    ...
    store(req,res,next){ 
        ...
        const paymentMethod = new paymentMethodModel(req.body);
        paymentMethod.save() 
            .then((paymentMethod) => { 
    ...
    edit(req,res,next){
        ...
        Promise.all( [
            ...
            .then(([paymentMethod]) => res.render(folder_view + '/edit',{
                title: module_name,
                item: mongooseToObject(paymentMethod),
                layout: layout,
                folder_module: folder_module,
                module_name: module_name,
                username: req.username,
    ...
    update(req,res,next){
        ...
        paymentMethodModel.updateOne({_id: req.params.id}, req.body, {runValidators: true})
            .then(() => {
                res.redirect( module_path)
            })
            .catch(error => {
    ...
    forceDestroy(req,res,next){
        ...
        paymentMethodModel.deleteOne({_id: req.params.id}).then().catch(next)
